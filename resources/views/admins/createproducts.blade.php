...
<style>
  ...
  .drop-zone .dz-text { margin: 0; color: #555; font-size: .95rem; }
  .drop-zone .dz-hint  { color: #999; font-size: .82rem; margin-top: 4px; }
  .drop-zone .click-link { color: #007bff; text-decoration: underline; }
            ...
            <div id="dropZone" class="drop-zone">
              ...
              <div id="dzDone" style="display:none;">
                ...
                <button type="button" id="changeImageBtn"
                        class="btn btn-sm btn-outline-secondary mt-2">Đổi ảnh</button>
              </div>
...
(function () {
  ...
  function setZoneState(state) {
    ...
    if (state === 'idle')      { dzIdle.style.display      = 'block'; }
    if (state === 'uploading') { dzUploading.style.display = 'block'; dropZone.classList.add('uploading'); }
    if (state === 'done')      { dzDone.style.display      = 'block'; dropZone.classList.add('upload-done'); }
  }
  ...
  function uploadFile(file) {
    if (!storage) { showError('Firebase chưa sẵn sàng.'); return; }
    hideError();
    imageUrlField.value = '';
    submitBtn.disabled  = true;
    ...
    var storageRef = storage.ref('products/' + fileName);
